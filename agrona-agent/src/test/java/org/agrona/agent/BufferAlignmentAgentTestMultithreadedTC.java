...
public class BufferAlignmentAgentTestMultithreadedTC extends MultithreadedTestCase
{
    ...
    @Test
    public void testUnsafeBufferFromByteArray() throws Throwable {
    	TestFramework.runManyTimes(new BufferAlignmentAgentTestMultithreadedTC(), 100);
    }
    ...
    private void assertUnaligned(final int index, final IntConsumer methodUnderTest)
    {
        ...
        catch (final BufferAlignmentException ex)
        {
            ...
            Assert.assertEquals(index, indexFound);
            assertNotEquals(0, offsetFound, "BufferAlignmentException reported wrong offset");

            return;
        }

        Assert.fail("Should have thrown BufferAlignmentException");
    }
}